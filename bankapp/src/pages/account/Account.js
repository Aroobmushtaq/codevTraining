...
//                     <td className="px-6 py-3 border-l border-white bg-blue-50">{new Date(account.createdAt).toLocaleString()}</td>
...
function Account() {
  const [accounts] = useState(() => JSON.parse(localStorage.getItem("accounts")) || []);
  const navigate = useNavigate();
            ...
            {accounts.length > 0 ? (
              accounts.map((account) => (
                <tr key={account.id} className="hover:bg-gray-100">
                  ...
                  <td className="px-4 py-2 bg-blue-50">{account.accountNumber}</td>
                  <td className="px-4 py-2 bg-blue-50">{account.fullName}</td>
                  <td className="px-4 py-2 bg-blue-50">{new Date(account.createdAt).toLocaleString()}</td>
        ...
        {accounts.length > 0 ? (
          accounts.map((account) => (
            <div key={account.id} className="border rounded-xl shadow-lg p-4 bg-white space-y-2">
              ...
              <p><span className="font-semibold">Registered:</span> {new Date(account.createdAt).toLocaleString()}</p>
